...
| `scoring.js` | Cooperative efficiency stars: 3 (first attempt, within both pars), 2 (≤2 attempts, ≤150% of both pars), 1 (any other solve). A sigil is one token (`expandedTokenCount` weighs it as its icons where needed). No failure state. Stars are computed from the reducer's append-only `roundStats` ledger, not the live transcript, so retracting a card cannot reduce the score. |
