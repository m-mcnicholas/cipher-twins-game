...
| `sigil-identity.js` | `identityFor(tokens)` → `{ glyphIndex, hue }`, a pure, stable look for a saved sigil derived from the icons it stands for (FNV-1a, cosmetic not concealment). Both clients render the same badge with no extra sync. `revision.sigilUses` tracks cumulative reuse per sigil for the end-screen recap. |
| `scoring.js` | Cooperative stars, rewarding *how* a pair solved: 3 = first-try mutual match **and** (reused a saved sigil this round **or** no language exists yet and stayed within par-slack); 2 = ≤2 attempts, or a first-try solve that met neither; 1 = any other solve. Raw token/message counts are a displayed-only stat, never a star gate. Thresholds (`EARLY_PAR_SLACK`, the reuse rule) are **provisional** pending WP-3 playtests. A sigil is one token for volume, its icons for weight (`expandedTokenCount`). No failure state. Scored from the append-only `roundStats` ledger, not the live transcript, so retracting a card cannot lower the score; nested sigils are rejected at proposal time. |
...
| `revision.js` | Versioned, host-authoritative shared state + the pure `reduce(revision, op, actor, context)`. Guarantees: version +1 per state-changing op, whole-card append order, author-only retraction, proposer + partner sigil approval, palette only grows, one attempt per resolved commitment pair, no secret ever stored (`snapshot()` throws otherwise). A tutorial advances only on mutual readiness **and** a complete `TUTORIAL_OBJECTIVES` checklist (`tutorial:skipVote` bypasses by mutual consent). `prepareRecovery` clears a half-finished commitment round and asks both players to recommit. |
